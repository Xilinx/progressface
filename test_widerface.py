...
sys.path.insert(0,'../retinaface/mxnet_v1.3/python/')
import mxnet as mx
...
from progressface import ProgressFace

def parse_args():
    parser = argparse.ArgumentParser(description='Test widerface by ProgressFace detector')
    # general
...
def get_boxes(roi, pyramid):
  ...
  boxes, landmarks, ct_boxes= detector.detect(im, threshold=args.thresh, scales = scales, do_flip=do_flip)
  ...
  if imgid>=0 and imgid<100:
    ...
    if config.centernet_branch and not config.ct_ab_concat_af:
        for i in xrange(ct_boxes.shape[0]):
            box = ct_boxes[i]
            ibox = box[0:4].copy().astype(np.int)
            cv2.rectangle(im, (ibox[0], ibox[1]), (ibox[2], ibox[3]), (0, 255, 255), 2)
...
def test(args):
  ...
  detector = ProgressFace(args.prefix, args.epoch, args.gpu, network=args.network, nocrop=args.nocrop, vote=args.bbox_vote)
  ...
  for i in xrange(len(roidb)):
    if i%args.parts!=args.part:#i = 48 is no ground truth
      continue
    #if i%10==0:
    #  print('processing', i, file=sys.stderr)
    roi = roidb[i]
    boxes, ct_boxes = get_boxes(roi, args.pyramid)
    if 'boxes' in roi:
    ...
    if config.centernet_branch and not config.ct_ab_concat_af:
        ...
        overlaps = bbox_overlaps(ct_boxes.astype(np.float), gt_boxes.astype(np.float))
        # print(im_info, gt_boxes.shape, boxes.shape, overlaps.shape, file=sys.stderr)

        _gt_overlaps = np.zeros((gt_boxes.shape[0]))
        if ct_boxes.shape[0]>0:
            _gt_overlaps = overlaps.max(axis=0)
            #print('max_overlaps', _gt_overlaps, file=sys.stderr)
            for j in range(len(_gt_overlaps)):
              if _gt_overlaps[j]>0.5:
            ...
            recall_all = float(overall[0])/overall[1]
            #print('recall_all', _recall, file=sys.stderr)
            print('[%d]'%i, 'ct_recall', recall, (gt_boxes.shape[0], ct_boxes.shape[0]), 'all:', recall_all, file=sys.stderr)
        else:
          print('[%d]'%i, 'detect %d faces'%boxes.shape[0])
    ...
    with open(out_file, 'w') as f:
      ...
      if config.centernet_branch:
          if config.ct_ab_concat_af:
            ...
            f.write("%d\n"%(boxes.shape[0] + ct_boxes.shape[0]))
      else:
          f.write("%d\n" % (boxes.shape[0]))
      for b in range(boxes.shape[0]):
        box = boxes[b]
        f.write("%d %d %d %d %g \n"%(box[0], box[1], box[2]-box[0], box[3]-box[1], box[4]))
      if not config.ct_ab_concat_af:
          if config.centernet_branch:
              for b in range(ct_boxes.shape[0]):
                  box = ct_boxes[b]
                  f.write("%d %d %d %d %g \n" % (box[0], box[1], box[2] - box[0], box[3] - box[1], box[4]))
  # for i in xrange(len(roidb)):
  #   if i%args.parts!=args.part:#i = 48 is no ground truth
  ...
  #       recall_all = float(overall[0])/overall[1]
  #       #print('recall_all', _recall, file=sys.stderr)
  #       print('[%d]'%i, 'recall', recall, (gt_boxes.shape[0], ct_boxes.shape[0]), 'all:', recall_all, file=sys.stderr)
  #   else:
  #     print('[%d]'%i, 'detect %d faces'%ct_boxes.shape[0])
  #
  #
  #   _vec = roidb[i]['image'].split('/')
  #   out_dir = os.path.join(output_folder, _vec[-2])
