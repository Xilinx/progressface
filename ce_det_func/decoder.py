...
def _nms(heat, kernel=3):
    pad = (kernel - 1) // 2
    hmax = nd.Pooling(data=heat, kernel= (kernel, kernel), stride=(1,1), pad=(pad,pad),pool_type='max') # default is max pooling
...
def _topk(scores, K=40):
    ...
    topk_ys = nd.reshape(topk_ys, (K,batch))

    topk_xs = _gather_feat(nd.reshape(topk_xs, (batch, -1, 1)), topk_ind)
    topk_xs = nd.reshape(topk_xs, (K,batch))

    return topk_score.asnumpy().astype(np.float32), topk_inds.asnumpy(), topk_clses.asnumpy(), topk_ys.asnumpy(), topk_xs.asnumpy()

def _topk_mining(scores, K=40):
    batch, cat, height, width = scores.shape
    ...
    return topk_scores.astype('float32'), topk_inds, topk_ys, topk_xs
