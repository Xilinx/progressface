...
def get_crop_batch(roidb): # return data, gt_boxes, gt_landmarks
    """
    prototype for rpn batch: data, im_info, gt_boxes
    :param roidb: ['image', 'flipped'] + ['gt_boxes', 'boxes', 'gt_classes']
    :return: data, label
...
class AA:
  def __init__(self, feat_shape, sel_stride, flag):
    ...
    A_list = []
    DEBUG = False
    min_stride = min(feat_strides)
    min_idx = feat_strides.index(min_stride)
    ...
    for i in range(len(feat_strides)):
        ...
        ratios = config.RPN_ANCHOR_CFG[sstride]['RATIOS']
        scales = config.RPN_ANCHOR_CFG[sstride]['SCALES']
        base_anchors = generate_anchors(base_size=base_size, ratios=list(ratios), scales=np.array(scales, dtype=np.float32), stride = min_stride, dense_anchor = config.DENSE_ANCHOR)
        num_anchors = base_anchors.shape[0]
        feat_height, feat_width = feat_shape[min_idx][-2:]
        feat_stride = feat_strides[i]
        feat_infos.append([feat_height, feat_width])
        ...
        all_anchors = anchors_plane(feat_height, feat_width, min_stride, base_anchors)
        all_anchors = all_anchors.reshape((K * A, 4))
        #print('anchor0', stride, all_anchors[0])
  ...
  def assign_anchor_fpn(self, gt_label, im_info, landmark=False, prefix='face', select_stride=0):
    ...
    if config.TRAIN.RPN_ENABLE_OHEM==0:
      my_label_list = []
      #all_fg_inds = np.where(labels == 1)
      for i in range(len(feat_strides)):
          _label = labels[sum(anchors_num_range[:i + 1]):sum(anchors_num_range[:i + 1]) + anchors_num_range[i + 1]]
          for j in range(A_list[i]):
              scale_indices = slice(j,_label.shape[0],A_list[i])
              scale_label = _label[scale_indices]

              fg_inds = np.where(scale_label == 1)[0]
              num_fg = int(config.TRAIN.RPN_FG_FRACTION * config.TRAIN.RPN_BATCH_SIZE / len(feat_strides)/A_list[i])
              if len(fg_inds) > num_fg:
                  disable_inds = npr.choice(fg_inds, size=(len(fg_inds) - num_fg), replace=False)
                  if DEBUG:
                      disable_inds = fg_inds[:(len(fg_inds) - num_fg)]
                  scale_label[disable_inds] = -1

              # subsample negative labels if we have too many
              num_bg = config.TRAIN.RPN_BATCH_SIZE/len(feat_strides)/A_list[i] - np.sum(scale_label == 1)
              bg_inds = np.where(scale_label == 0)[0]
              if len(bg_inds) > num_bg:
                  disable_inds = npr.choice(bg_inds, size=(len(bg_inds) - num_bg), replace=False)
                  if DEBUG:
                      disable_inds = bg_inds[:(len(bg_inds) - num_bg)]
                  scale_label[disable_inds] = -1
              my_label_list.append(scale_label)
      ...
      #        disable_inds = bg_inds[:(len(bg_inds) - num_bg)]
      #    labels[disable_inds] = -1
      labels = np.concatenate(my_label_list)
    else:
      fg_inds = np.where(labels == 1)[0]
      num_fg = len(fg_inds)
      bg_inds = np.where(labels == 0)[0]
