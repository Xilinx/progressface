...
class FocalLossOperator(mx.operator.CustomOp):
    ...
    def backward(self, req, out_grad, in_data, out_data, in_grad, aux):
        y_numpy = out_data[0].asnumpy().transpose((0,2,1)) #[batchsize,2,num_anchor] => [batchsize, num_anchor,2]
        #print(out_data[0].shape)
        label_numpy = in_data[1].asnumpy()
        y_numpy = y_numpy.reshape((-1,2))
        label_numpy = label_numpy.reshape((-1))#[batchsize,num_anchor] => [batchsize * num_anchor]
        #print(len(np.where(label_numpy == -1)[0]))
        indices = np.where(label_numpy.ravel() == -1)[0]
        #print(label_numpy)
        label_numpy[indices] = 1
