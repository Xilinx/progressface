...
class RPNFPNOHEM3Operator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        # bbox_outside_weight = np.zeros((labels_raw.shape[0], labels_raw.shape[1], 4), dtype=np.float32)
        ...
        for ibatch in xrange(labels_raw.shape[0]):
          ...
          # _bbox_outside_weight = np.zeros( (labels_raw.shape[1],4), dtype=np.float32)
          ...
          # _bbox_outside_weight[labels == 1, :] = 1.0 / num_examples
          # _bbox_outside_weight[labels == 0, :] = 1.0 / num_examples
          # bbox_outside_weight[ibatch] = _bbox_outside_weight
