...
class OHEGSOperator(mx.operator.CustomOp):
    def __init__(self, stride=0, network='', dataset='', prefix=''):
        super(OHEGSOperator, self).__init__()
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        for stride in feat_strides:
            anchor_num_list.append(labels_raw.shape[1])
...
@mx.operator.register('OHEGS')
class OHEGSProp(mx.operator.CustomOpProp):
    def __init__(self, stride=0, network='', dataset='', prefix=''):
        super(OHEGSProp, self).__init__(need_top_grad=False)
        self.stride = stride
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return OHEGSOperator(self.stride, self.network, self.dataset, self.prefix)
