...
class FocalLossOperator(mx.operator.CustomOp):
    ...
    def backward(self, req, out_grad, in_data, out_data, in_grad, aux):
        ...
        dx = self._alpha * np.power(1 - pt, self._gamma - 1) * (
                    self._gamma * (-1 * pt * pro_) * np.log(pt) + pro_ * (1 - pt)) * 1.0

        ####i==j
        # reload pt
        pt = self._pt + 1e-14
        dx[np.arange(pro_.shape[0], dtype='int'), labels.astype('int')] = self._alpha * np.power(1 - pt,
                                                                                                 self._gamma) * (
                                                                                      self._gamma * pt * np.log(
                                                                                  pt) + pt - 1) * (1.0)
