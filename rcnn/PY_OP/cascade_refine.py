...
class STROperator(mx.operator.CustomOp):
    def __init__(self, stride=0, network='', dataset='', prefix=''):
        ...
        self.mode = config.TRAIN.RPN_ENABLE_OHEM #0 for random 10:245, 1 for 10:246, 2 for 10:30, mode 1 for default
        stride = self.stride
        sstride = str(stride)
    ...
    def assign_anchor_fpn(self, gt_label, anchors, landmark=False, prefix='face'):
      ...
      if landmark:
        landmark_target = landmark_targets.reshape((1, feat_height*feat_width, A * landmark_pred_len)).transpose(0, 2, 1)
        # landmark_target /= config.TRAIN.LANDMARK_STD
        landmark_weight = landmark_weights.reshape((1, feat_height*feat_width, A * landmark_pred_len)).transpose((0, 2, 1))
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        gt_landmarks = in_data[7].asnumpy()
        landmark_target_t0 = in_data[8].asnumpy()
        ...
        landmark_target_out = np.zeros(shape=landmark_target_t0.shape, dtype=np.float32)
        ...
        for ibatch in range(batch_size):
          ...
          if not FAST:
            ...
            _gt_landmarks = gt_landmarks[ibatch]
            #print('gt num', _gtind)
            _gt_boxes = _gt_boxes[0:_gtind,:]
            _gt_landmarks = _gt_landmarks[0:_gtind,:,:]
            ...
            if config.FACE_LANDMARK:
                gt_label['gt_landmarks'] = _gt_landmarks
            new_label_dict = self.assign_anchor_fpn(gt_label, anchors_t1, config.FACE_LANDMARK, prefix=self.prefix)
            labels = new_label_dict['%s_label'%self.prefix] #AHW
            new_bbox_target = new_label_dict['%s_bbox_target'%self.prefix] #AC,HW
            new_landmark_target = new_label_dict['%s_landmark_target'%self.prefix]
            ...
            landmark_target_out[ibatch] = new_landmark_target
        ...
        for ind, val in enumerate([labels_out, bbox_target_out, landmark_target_out, anchor_weight, valid_count]):
            val = mx.nd.array(val)
...
class STRProp(mx.operator.CustomOpProp):
    ...
    def list_arguments(self):
        ...
        return ['cls_label_t0', 'cls_pred_t0', 'cls_pred', 'bbox_pred_t0', 'bbox_label_t0', 'cls_label_raw', 'cas_gt_boxes', 'cas_gt_landmarks', 'landmark_target_t0']

    def list_outputs(self):
        return ['cls_label_out', 'bbox_label_out','landmark_target_out', 'anchor_weight_out', 'pos_count_out']
    ...
    def infer_shape(self, in_shape):
        ...
        landmark_shape = [bs, 30, in_shape[4][2]]
        return in_shape, \
               [cls_label_shape, in_shape[4],landmark_shape, [bs,num_anchors,1], [bs,1]]

    def create_operator(self, ctx, shapes, dtypes):
        return STROperator(self.stride, self.network, self.dataset, self.prefix)
