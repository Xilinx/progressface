...
# if config.USE_OHEGS:
#     from rcnn.io.rpn_grouped import get_rpn_testbatch, get_rpn_batch, assign_anchor_fpn, get_crop_batch, AA
# else:
...
class CropLoader(mx.io.DataIter):
    def __init__(self, feat_sym, roidb, sel_stride = [], flag = False, batch_size=1, shuffle=False, ctx=None, work_load_list=None,
        ...
        if config.STR:
            ...
            if config.FACE_LANDMARK:
                self.label_name.append('gt_landmarks')
        if config.centernet_branch: # for every stride construct anchor-free branch label
            for stride in config.ct_stride:
                self.label_name.append('ct_hm_stride_%s'%stride)
                self.label_name.append('ct_wh_stride_%s'%stride)
                self.label_name.append('ct_wh_mask_stride_%s'%stride)
                if config.ct_offset:
                    self.label_name.append('ct_offset_stride_%s' % stride)
                if config.ct_wh_inds:
                    self.label_name.append('ct_ind_stride_%s' % stride)
                if config.ct_landmarks:
                    self.label_name.append('ct_landmark_stride_%s' % stride)
                if config.ct_mining:
                    self.label_name.append('gt_boxes')
                if config.ct_head:
                    self.label_name.append('ct_head_hm_stride_%s' % stride)
                    self.label_name.append('ct_head_wh_stride_%s' % stride)
                    self.label_name.append('ct_head_wh_mask_stride_%s' % stride)
                    if config.ct_offset:
                        self.label_name.append('ct_head_offset_stride_%s' % stride)
                    if config.ct_wh_inds:
                        self.label_name.append('ct_head_ind_stride_%s' % stride)
                    if config.ct_landmarks:
                        self.label_name.append('ct_head_landmark_stride_%s' % stride)
                    # self.label_name.append('overlaps')
        ...
        self.aa = AA(feat_shape_list, sel_stride, flag)
    ...
    def infer_shape(self, max_data_shape=None, max_label_shape=None):
        ...
        if config.STR:
            ...
            if config.FACE_LANDMARK:
                label_dict['gt_landmarks'] = np.zeros((0, config.TRAIN.MAX_BBOX_PER_IMAGE, 5 , 3), dtype=np.float32)
    ...
    def get_batch(self):
        ...
        for data, label in zip(data_list, label_list):
            ...
            if config.centernet_branch:
                gt_centers = label['gt_centers']
                gt_label['gt_centers'] = gt_centers
            if config.HEAD_BOX:
              gt_label['gt_boxes_head'] = label['gt_boxes_head']
            ...
            # if not face_label_dict.has_key('ct_hm_stride_8'):
            #     print('sss')
            #tb = datetime.datetime.now()
            #self._times[0] += (tb-ta).total_seconds()
            # if config.centernet_branch and config.ct_mining:
            #     overlaps = face_label_dict['overlaps']
            #     del face_label_dict['overlaps']
            label_dict.update(face_label_dict)
            ...
            #             self.label_name.append(tmp_name)
            ...
            if config.centernet_branch and config.ct_mining:
                ...
                # pad_overlaps = np.empty((1, self.aa.inds_inside.size,config.TRAIN.MAX_BBOX_PER_IMAGE), dtype=np.float32)
                # pad_overlaps.fill(0)
                # pad_overlaps[0,:,0:overlaps.shape[1]] = overlaps
                # label_dict['overlaps'] = pad_overlaps
            if config.STR:   #padding the gt_label
                pad_gt_boxes = np.empty((1, config.TRAIN.MAX_BBOX_PER_IMAGE, 5), dtype=np.float32)
                pad_gt_boxes.fill(-1)
                pad_gt_boxes[0, 0:gt_boxes.shape[0], :] = gt_boxes
                label_dict['gt_boxes'] = pad_gt_boxes
                if config.FACE_LANDMARK:
                    pad_gt_landmarks = np.empty((1, config.TRAIN.MAX_BBOX_PER_IMAGE, 5, 3), dtype=np.float32)
                    pad_gt_landmarks.fill(-1)
                    pad_gt_landmarks[0, 0:gt_landmarks.shape[0], :, :] = gt_landmarks
                    label_dict['gt_landmarks'] = pad_gt_landmarks
                # gt_boxes_tmp = np.full((1,2000,5),-1)
                # gt_landmarks_tmp = np.full((1,2000,5,3),-1)
                # for i in range(gt_boxes.shape[0]):
                #     gt_boxes_tmp[0,i,:] = gt_boxes[i,:]
                # for i in range(gt_landmarks.shape[0]):
                #     gt_landmarks_tmp[0,i,:,:] = gt_landmarks[i,:,:]
                # label['gt_boxes'] = gt_boxes_tmp
                # label['gt_landmarks'] = gt_landmarks_tmp
            for k in self.label_name:
              # if config.STR:
              #   if k in ['gt_boxes', 'gt_landmarks']:
              #     continue
