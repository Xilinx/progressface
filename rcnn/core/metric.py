...
class SmooothL1LossMetric(mx.metric.EvalMetric):
    def __init__(self, loss_idx=-1, weight_idx=-1, name='SmoothL1Loss'):
        super(SmooothL1LossMetric, self).__init__(name)
        self.pred, self.label = get_rpn_names()
        self.loss_idx = loss_idx
        self.weight_idx = weight_idx
        self.name = name
    ...
    def update(self, labels, preds):
        ...
        self.sum_metric += np.sum(bbox_loss)
        self.num_inst += num_inst
class KLLossMetric(mx.metric.EvalMetric):
    def __init__(self, loss_idx=-1, weight_idx=-1, name='KLLoss'):
        super(KLLossMetric, self).__init__(name)
        self.pred, self.label = get_rpn_names()
        self.loss_idx = loss_idx
        self.weight_idx = weight_idx
        self.name = name
    ...
    def update(self, labels, preds):
        ...
        if config.try_end2end:
            num_inst = np.sum(bbox_weight > 0) / (bbox_weight.shape[0] / config.NUM_ANCHORS)
        else:
            num_inst = np.sum(bbox_weight > 0) / (bbox_weight.shape[1]/config.NUM_ANCHORS)
        #print('in_metric log', bbox_loss.shape, num_inst, file=sys.stderr)
        ...
        self.sum_metric += np.sum(bbox_loss)
        self.num_inst += num_inst
class pure_loss(mx.metric.EvalMetric):
    def __init__(self, loss_idx=-1, name='RPNL1Loss'):
        super(pure_loss, self).__init__(name)
        self.pred, self.label = get_rpn_names()
        self.loss_idx = loss_idx
        self.name = name
    ...
    def update(self, labels, preds):
        ...
        #print(bbox_loss.sum())
        # print('sss')
        # print(bbox_loss.size)
        self.sum_metric += np.sum(bbox_loss)
        self.num_inst += bbox_loss.size
class FocalLoss(mx.metric.EvalMetric):
    def __init__(self, pred_idx=-1,label_idx=-1,name='focal_loss'):
    ...
    def update(self, labels, preds):
        ...
        #pred_label = mx.ndarray.argmax_channel(pred).asnumpy().astype('int32')
        # pred_label = pred_label.reshape((pred_label.shape[0], -1))
        pred_label = mx.nd.array(pred.asnumpy().reshape(-1, 2))
        # label (b, p)
        label = label.asnumpy().astype('int32').reshape(-1, )
        # print(pred_label.shape, label.shape)
        ...
        _pred_label = pred_label[keep_inds]
        _label = label[keep_inds]
        assert _label.shape[0] == _pred_label.shape[0]

        prob = _pred_label[np.arange(_label.shape[0]), np.int64(_label)]
        self.sum_metric += self.alpha * (-1.0 * np.power(1.0-prob, self.gamma) * np.log(prob + self.eps)).sum()
        self.num_inst += _label.shape[0]
